...
def obtener_datos_tarea(id_tarea):
    """
    Obtiene todos los datos necesarios de una tarea específica.
    Args:
    ...
    try:
        ...
        """)
        result = session.execute(query, {"id_tarea": id_tarea})
        row = result.fetchone()
...
def export_tarea_individual_to_excel(file_path, id_tarea):
    """
    Genera un Excel con el resumen de una tarea realizada.
    Args:
    ...
    try:
        ...
        first_table = Table(displayName="ResumenTareaRealizada", ref=f"A{first_table_first_row}:K{first_table_last_row}")
